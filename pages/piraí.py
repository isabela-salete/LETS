...
df2 = pd.read_csv('pirai.csv')
df2.drop(['Unnamed: 0'], axis=1, inplace= True)

st.title('Análise no município de Piraí')
st.dataframe(df2)
...
st.text('CBO')
st.bar_chart(data = df2['CBO'], width=500, height= 400)


st.text('CID')
st.bar_chart(data = df2['CID-10'], width=500, height= 400)